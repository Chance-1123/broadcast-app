...
function WeeklyGrid({rows,conflicts,monday,onEdit,onCancel}){
  ...
  function getBlocksInHour(dayDate,hourStr){
    ...
    return rows.map((r,i)=>({...r,idx:i})).filter(r=>{
      ...
      <table style={{width:"100%",borderCollapse:"collapse",tableLayout:"fixed"}}>
        <thead>
          <tr style={{position:"sticky",top:0,zIndex:2}}>
            <th style={{width:56,padding:"8px 4px",background:"#f8f8f6",borderBottom:"2px solid #e5e5e3",borderRight:"0.5px solid #e5e5e3",fontSize:11,fontWeight:600,color:"#888",textAlign:"center"}}>시간</th>
            {DAYS.map((day,di)=>{
              const isToday=fmtFull(new Date())===dayDates[di];
              const dayCount=rows.filter(r=>r.날짜===dayDates[di]&&r.장소).length;
              return(
                <th key={day} style={{padding:"8px 4px",background:isToday?"#F0FBF6":"#f8f8f6",borderBottom:"2px solid #e5e5e3",borderRight:"0.5px solid #e5e5e3",fontSize:12,fontWeight:700,color:isToday?"#1D9E75":"#222",textAlign:"center"}}>
                  <div>{day}요일</div>
                  <div style={{fontSize:10,fontWeight:400,color:isToday?"#1D9E75":"#aaa"}}>{fmtShort(addDays(monday,di))}</div>
                  {dayCount>0&&<div style={{fontSize:9,color:isToday?"#1D9E75":"#aaa"}}>{dayCount}건</div>}
                </th>
              );
            })}
          </tr>
        ...
        <tbody>
          {HOURS.map((hour,hi)=>{
            ...
            const isNowHour=parseInt(hour)===nowH&&fmtFull(new Date())===dayDates[0]||false;
            return(
                ...
                {DAYS.map((day,di)=>{
                  ...
                  return(
                    <td key={day} style={{padding:"3px 4px",borderBottom:"0.5px solid #f0f0ee",borderRight:"0.5px solid #e5e5e3",verticalAlign:"top",background:isToday?"rgba(29,158,117,0.02)":"transparent"}}>
                      {allBlocks.length===0&&<div style={{height:28,display:"flex",alignItems:"center",justifyContent:"center",color:"#f0f0ee",fontSize:10}}>·</div>}
                      {continueBlocks.map((b,bi)=>{
                        const color=getColor(b.장소);
                        return(
                          <div key={`c${bi}`} style={{borderLeft:`3px solid ${color}`,background:`${color}08`,borderRadius:"0 4px 4px 0",padding:"2px 5px",marginBottom:2,cursor:"pointer",opacity:0.65}}
                      ...
                      {startBlocks.map((b,bi)=>{
                        ...
                        return(
                          <div key={bi}
                            style={{background:`${color}12`,border:`1.5px solid ${color}`,borderLeft:`3px solid ${color}`,borderRadius:5,padding:"4px 5px",marginBottom:3,cursor:"pointer",transition:"box-shadow 0.15s"}}
                          ...
                          >
                            ...
                            <div style={{fontSize:9,fontWeight:700,color,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{b.장소}</div>
                            <div style={{fontSize:9,fontWeight:600,color:"#333",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{isPrep?"📋 방송준비":shortContent||b.주제||b.내용}</div>
...
function LiveBanner({rows}){
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:12,background:`${color}10`,border:`1.5px solid ${color}40`,borderRadius:10,padding:"10px 16px",marginBottom:10,overflow:"hidden",position:"relative"}}>
      ...
      {live.length>1&&<div style={{position:"absolute",right:16,top:6,fontSize:10,color:"#aaa"}}>{rollIdx%live.length+1}/{live.length}</div>}
    </div>
  );
}
    ...
    <div style={{display:"flex",flexDirection:"column",width:"100vw",height:"100vh",fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",fontSize:14,background:"#f5f5f4",color:"#1c1c1a",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 18px",height:50,background:"#fff",borderBottom:"0.5px solid #e5e5e3",flexShrink:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <button style={{...btn,color:"#A32D2D",borderColor:"#F09595",fontSize:12,height:32}} onClick={resetAllData}>🗑 초기화</button>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        {/* sidebar */}
        <nav style={{width:185,background:"#fff",borderRight:"0.5px solid #e5e5e3",padding:"12px 8px",overflowY:"auto",flexShrink:0}}>
          <div style={{fontSize:10,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:"0.6px",padding:"0 8px",marginBottom:6}}>메뉴</div>
          {[
            {id:"dashboard",icon:"📊",label:"대시보드"},
            {id:"upload",   icon:"📤",label:"편성표 업로드"},
            {id:"schedule", icon:"📅",label:"스케줄 목록"},
            {id:"notifications",icon:"🔔",label:`알림${unread>0?` (${unread})`:""}`},
          ].map(item=>(
            <div key={item.id} style={{display:"flex",alignItems:"center",gap:7,padding:"6px 8px",borderRadius:8,fontSize:12,cursor:"pointer",color:tab===item.id?"#1c1c1a":"#666",fontWeight:tab===item.id?600:400,background:tab===item.id?"#f0f0ee":"transparent",marginBottom:2}} onClick={()=>setTab(item.id)}>
              <span>{item.icon}</span>{item.label}
            </div>
          ))}
          <div style={{margin:"10px 8px 6px",borderTop:"0.5px solid #e5e5e3"}}></div>
          <div style={{fontSize:10,fontWeight:600,color:"#999",textTransform:"uppercase",letterSpacing:"0.6px",padding:"0 8px",marginBottom:6}}>
            {studioStats.length>0?"스튜디오 현황":"스튜디오 (11개)"}
          </div>
          {(studioStats.length>0?studioStats:STUDIOS.map(s=>({id:s.id,color:s.color,count:null,cf:0}))).map(st=>(
            <div key={st.id} style={{display:"flex",alignItems:"center",padding:"4px 8px",fontSize:11,color:st.count!=null?"#555":"#ccc",borderRadius:6,cursor:"pointer"}} onClick={()=>{setTab("schedule");setFilterStudio(st.id);}}>
              ...
              <div style={{display:"flex",gap:12,height:"calc(100vh - 220px)",minHeight:400}}>
                ...
                <div style={{width:"32%",flexShrink:0,display:"flex",flexDirection:"column",gap:8}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <div style={{fontSize:13,fontWeight:700,color:"#1D9E75"}}>📅 오늘 스케줄</div>
                    <div style={{fontSize:11,color:"#aaa"}}>{fmtShort(new Date())} ({["일","월","화","수","목","금","토"][new Date().getDay()]}요일)</div>
                  </div>
                  <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:6}}>
                    {(()=>{
                      const todayStr=fmtFull(new Date());
                      const todayRows=rows.map((r,i)=>({...r,idx:i})).filter(r=>r.날짜===todayStr&&r.장소&&r.시작시간).sort((a,b)=>toMin(a.시작시간)-toMin(b.시작시간));
                              ...
                              {sRows.map((b,bi)=>{
                                ...
                                return(
                                  <div key={bi} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 8px",borderRadius:6,background:isCf?"#FFF8F8":isPrep?"#F8FFF8":"#f8f8f6",border:`0.5px solid ${isCf?"#F09595":isPrep?`${color}30`:"#f0f0ee"}`,cursor:"pointer"}} onClick={()=>setBookingModal(b.idx)}>
                                    ...
                                    <div style={{flex:1,minWidth:0}}>
                                      <div style={{fontSize:11,fontWeight:600,color:isCf?"#E24B4A":"#222",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{isCf&&"⚠ "}{isPrep?"📋 방송준비":shortContent||content}</div>
            ...
            <>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <div style={panel}>
                  ...
                  <div style={{fontSize:11,color:"#888",marginBottom:12}}>VBA로 생성한 방송편성표 엑셀 → Supabase DB 저장</div>
                  <div style={{border:uploadState==="done"?"1.5px solid #1D9E75":"1px dashed #ccc",borderRadius:10,padding:24,textAlign:"center",cursor:"pointer",marginBottom:8}} onClick={()=>fileRef.current.click()} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();handleFile({target:{files:e.dataTransfer.files}});}}>
                    ...
                    {filteredRows.map((row,i)=>{
                      ...
                      return(
                        <tr key={i} style={{borderBottom:"0.5px solid #f0f0ee",background:isCf?"#FFF8F8":noSt?"#FFFBF0":"#fff"}}>
                          ...
                          {td(row.시작시간)}{td(row.종료시간)}{td(row.길이)}
                  ...
                  <div style={{flex:1}}>
                    ...
                    {n.type==="upload"&&(
                      <div style={{marginTop:6,display:"flex",gap:6,alignItems:"center"}}>
                        ...
                        <button style={{...btn,height:24,fontSize:11}} onClick={()=>setNotifs(prev=>prev.map(x=>x.id===n.id?{...x,unread:false}:x))}>확인</button>
                      </div>
                    )}
                  </div>
                  <div style={{fontSize:10,color:"#aaa",flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                    {n.unread&&!n.resolved&&<span style={{...bd("red"),padding:"1px 5px",fontSize:9}}>new</span>}
                    {n.time}
                  </div>
                </div>
              ))}
